...
export function initAnchor({ icloudBase, dirName = 'ianchor', interactive = true }) {
  const configDir = ensureConfigDir(icloudBase, dirName);
  const db = openDb(configDir);

  // Scan existing files (excluding ianchor.db, .DS_Store, and image files)
  const items = fs.readdirSync(configDir).filter(
    name => !name.startsWith('ianchor.db') && !shouldIgnore(name)
  );
  let discovered = 0;
